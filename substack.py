...
SCHEDULE_STATE_FILE = os.path.join(os.path.dirname(__file__), ".substack_schedule_state.json")
...
def download_image(url):
    # timeout=(connect_s, read_s) — read timeout resets per chunk, so cap total via stream
    r = requests.get(url, timeout=(10, 60), stream=True)
    r.raise_for_status()
    chunks = []
    total = 0
    for chunk in r.iter_content(chunk_size=65536):
        chunks.append(chunk)
        total += len(chunk)
        if total > 20 * 1024 * 1024:  # 20 MB hard cap
            break
    data = b"".join(chunks)
...
def load_last_slot():
    """Return the last successfully scheduled post datetime, or None."""
    if os.path.exists(SCHEDULE_STATE_FILE):
        with open(SCHEDULE_STATE_FILE) as f:
            d = json.load(f)
        return datetime.fromisoformat(d["last_slot"]).replace(tzinfo=timezone.utc)
    return None


def save_last_slot(dt):
    with open(SCHEDULE_STATE_FILE, "w") as f:
        json.dump({"last_slot": dt.isoformat()}, f)


def next_weekday_slots(count, after_dt=None, hour_utc=1):
    """Return `count` Mon–Fri datetimes at hour_utc UTC, starting the next weekday after after_dt."""
    base = after_dt or datetime.now(timezone.utc)
    day = (base + timedelta(days=1)).replace(hour=hour_utc, minute=0, second=0, microsecond=0)
    slots = []
    while len(slots) < count:
        if day.weekday() < 5:   # Mon=0 … Fri=4; skip Sat=5 and Sun=6
            slots.append(day)
...
def main():
    ...
    last_slot = load_last_slot()
    if last_slot:
        print(f"  Resuming after last scheduled slot: {last_slot.strftime('%a %d %b %Y %H:%M UTC')}")
    else:
        print("  No prior state — starting from tomorrow (Mon–Fri only)")

    slots = next_weekday_slots(len(pages), after_dt=last_slot, hour_utc=1)
