...
Skips items that already have a valid SKU unless it is blank, starts with UNB,
or uses UNK as the material code, which are regenerated.
...
def should_regenerate(existing):
    if not existing:
        return True
    parts = existing.upper().split("-")
    brand_code = parts[0] if parts else ""
    material_code = parts[2] if len(parts) >= 3 else ""
    return brand_code == "UNB" or material_code == "UNK"


def has_unk_material(existing):
    parts = existing.upper().split("-")
    return len(parts) >= 3 and parts[2] == "UNK"
...
def build_sku(page, counters):
    ...
    # Skip items that already have a non-placeholder SKU
    if not should_regenerate(existing):
...
def main():
    only_material_unk = "--only-material-unk" in sys.argv[1:]
    ...
    if only_material_unk:
        needs_sku = [x for x in needs_sku if has_unk_material(x["existing"])]
    has_sku    = [x for x in sku_list if x["skip"]]
    redo_unb   = [x for x in needs_sku if x["existing"].upper().startswith("UNB")]
    redo_unk   = [x for x in needs_sku if has_unk_material(x["existing"])]
    ...
    print(f"    — Redo (UNK):         {len(redo_unk)}")
    if only_material_unk:
        print("    — Mode:               only material-UNK rows")
